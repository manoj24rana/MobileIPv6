...
//assuming interface 1 as the active interface
   
                Ptr<Ipv6> ip = node->GetObject<Ipv6> ();
                Ipv6InterfaceAddress ads = ip->GetAddress(1,0);                
                m_buinf->SetCoa(ads.GetAddress());
  
  std::cout<<"IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII"<<ads.GetAddress();
  //m_buinf->SetHomeAgentList(m_Haalist);
  if (m_Haalist.size())
  {
   m_buinf->SetHA(m_Haalist.front());
   (m_buinf->GetHA()).GetBytes(buf1);
   (m_buinf->GetCoa()).GetBytes(buf2);
   for (i=0;i<8;i++)
    buf[i]=buf1[i];
   for (i=0;i<8;i++)
    buf[i+8]=buf2[i+8];    
   ...
   m_buinf->SetHoa(addr);
Ptr<Ipv6TunnelL4Protocol> tunnel4prot = node->GetObject<Ipv6TunnelL4Protocol> ();  
  ...
  Ptr<UdpL4Protocol> udpl4 = node->GetObject<UdpL4Protocol> ();
  udpl4->SetMipv6Callback(MakeCallback(&BList::GetHoa, m_buinf));

  Ptr<TcpL4Protocol> tcpl4 = node->GetObject<TcpL4Protocol> ();
  ...
  {
        m_buinf = CreateObject<BList> (m_Haalist);
        m_buinf->SetNode (GetNode ());
  Ipv6Address coa;             
  Ptr<Node> node = this->GetObject<Node> ();
  Ptr<Ipv6> ip = node->GetObject<Ipv6> ();
  Ipv6InterfaceAddress ads = ip->GetAddress(1,0);                
  coa=ads.GetAddress();
  std::cout<<"RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR"<<coa;               
                
  if (coa!=m_buinf->GetCoa())
  {                
  m_buinf->SetCoa(coa);
  //m_buinf->SetHomeAgentList(m_Haalist);
  if (m_Haalist.size())
  ...
  Ptr<Icmpv6L4Protocol> icmpv6l5 = node->GetObject<Icmpv6L4Protocol> ();
  icmpv6l5->SetNewIPCallback(MakeCallback(&mipv6MN::HandleNewAttachment, this));
...
void mipv6MN::HandleNewAttachment (Ipv6Address ipr)
{
  ...
  //send BU
NS_LOG_FUNCTION ("LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL"<<m_buinf->GetHA ());
...
// Options are not implemented yet!!
  ...
  switch (ba.GetStatus ())
    ...
    case MIPv6Header::BA_STATUS_BINDING_UPDATE_ACCEPTED:
    {
	  ...
	  if (IsRouteOptimizationRequired() && !m_buinf->GetCN().IsEqual("0::0"))
	  {
